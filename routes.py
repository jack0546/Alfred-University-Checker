...
from utils import GradeValidator, ResultSlipProcessor, AdmissionMatcher
...
def upload_result():
    """Handle result slip upload with automatic processing"""
    if request.method == 'POST':
        # Handle file upload
        if 'result_slip' not in request.files:
            return jsonify({'success': False, 'error': 'No file selected'}), 400
        
        file = request.files['result_slip']
        if file.filename == '':
            return jsonify({'success': False, 'error': 'No file selected'}), 400
        
        if not ResultSlipProcessor.allowed_file(file.filename):
            return jsonify({'success': False, 'error': 'Invalid file type. Use PDF, JPG, or PNG'}), 400
        
        # Save file
        filepath = ResultSlipProcessor.save_upload_file(file)
        if not filepath:
        ...
        if filepath.lower().endswith(('.jpg', '.jpeg', '.png')):
            ocr_text, ocr_error = ResultSlipProcessor.extract_text_from_image(filepath)
            if ocr_text:
                # Parse the OCR text automatically
                extracted_data = ResultSlipProcessor.parse_result_slip_text(ocr_text)
        
        if not extracted_data:
            # OCR failed or not used, will use manual entry
            return jsonify({
                'success': True,
                'filepath': filepath,
                'auto_extract': False,
                'message': 'File uploaded. Please verify/enter grades manually.',
                'ocr_error': ocr_error
            })
        
        # Try automatic admission processing
        name = extracted_data.get('name', '')
        category = extracted_data.get('category', '')
        core_subjects = extracted_data.get('core_subjects', {})
        electives = extracted_data.get('electives', {})
        
        # Validate extracted data
        if not name or not category or len(core_subjects) < 3 or len(electives) < 3:
            # Not enough data extracted, ask for manual verification
            return jsonify({
                'success': True,
                'filepath': filepath,
                'auto_extract': False,
                'extracted_data': extracted_data,
                'message': 'Please review and complete the extracted information.',
            })
        
        # All data extracted successfully, process automatically
        try:
            ...
            if not all_passed:
                return jsonify({
                    'success': False,
                    'error': f'Failed core subjects: {", ".join(failed)}. Cannot calculate admission.'
                }), 400
            ...
            if not success:
                return jsonify({'success': False, 'error': f'Error: {error_msg}'}), 400
            ...
            return jsonify({
                ...
                'message': f'Admission calculated! Found {len(qualified)} qualified and {len(borderline)} borderline programs.'
            })
